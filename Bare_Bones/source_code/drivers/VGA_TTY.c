...
uint8_t defualt_color;
uint16_t defualt_text;
...
void terminal_initialize(void)
{
	...
	terminal_row = 0;
	terminal_column = 0;
	terminal_color = defualt_color;
	terminal_buffer = (uint16_t*) 0xB8000;
	for (size_t y = 0; y < VGA_HEIGHT; y++) {
		for (size_t x = 0; x < VGA_WIDTH; x++) {
			const size_t index = y * VGA_WIDTH + x;
			terminal_buffer[index] = defualt_text;
		}
	}
}
...
void terminal_center_writing(const char* data, const uint8_t color){
	...
	size_t center = (40 - terminal_column) - (lenth / 2);
	for(size_t i = 0 ; i < center; i++){
		terminal_writestring(".", defualt_color);
	}
	terminal_writestring(data, color);
}
void terminal_endl(const char* data, const uint8_t color){
		size_t lenth = strlen(data);
		terminal_column = 80 - lenth;
		terminal_writestring(data, color);
}
